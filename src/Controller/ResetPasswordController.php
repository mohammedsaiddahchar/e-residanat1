...
class ResetPasswordController extends AbstractController
{
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer, TranslatorInterface $translator): RedirectResponse
    {
        ...
        $subject =  'Password Reset Request on PGI-FMPT';

        if($user_lang =='fr-FR'){
            $subject =  'Demande de Réinitialisation de Mot de Passe sur PGI-FMPT';
        }
        if($user_lang =='ar-AR'){
            $subject =  'PGI-FMPT طلب إعادة تعيين كلمة المرور على ';
        }
        if($user_lang =='es-ES'){
            $subject =  'Solicitud de Restablecimiento de Contraseña en PGI-FMPT';
        }
        if($user_lang =='en-GB'){
            $subject =  'Password Reset Request on PGI-FMPT';
        }
        ...
        $email = (new TemplatedEmail())
            ->from(new Address('fmpt@uae.ac.ma', 'PGI_FMPT Mailer'))
            ->to($user->getEmail())
            ->subject($subject)
